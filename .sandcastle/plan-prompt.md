...
!`gh issue list --state open --label ready-for-agent --json number,title,body,labels,comments,state,closedAt --jq '[.[] | select(.state == "OPEN" and .closedAt == null) | {number, title, body, labels: [.labels[].name], comments: [.comments[].body]}]'`
